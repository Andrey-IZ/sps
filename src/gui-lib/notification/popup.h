...
class GUILIBSHARED_EXPORT PopUp : public QWidget
{
    Q_OBJECT
...
public slots:
    void setCanvasColor(QBrush backgroundColor = QBrush(QColor(0,0,0,180)), QPen pen = Qt::NoPen);
    void setPopupText(const QString& text, QString stylesheet = ""); // Установка текста в уведомление
    void show(int timeout = 3000);           /* Собственный метод показа виджета
