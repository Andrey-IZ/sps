...
PopUp::PopUp(QWidget *parent) : QWidget(parent),
    backgroundColor(QBrush()), backgroundPen(Qt::NoPen)
{
    ...
    connect(&animation, SIGNAL(finished()), this, SLOT(hide())); /* Подключаем сигнал окончания
                                                                             * анимации к слоты скрытия
                                                                             * */

    // Настройка текста уведомления
    label.setAlignment(Qt::AlignLeft | Qt::AlignVCenter); // Устанавилваем по центру
    // И настраиваем стили
...
void PopUp::paintEvent(QPaintEvent *event)
{
    ...
    painter.setPen(backgroundPen); // Край уведомления не будет выделен

    // Отрисовываем фон с закруглением краёв в 10px
    painter.drawRoundedRect(roundedRect, 10, 10);
}

void PopUp::setCanvasColor(QBrush backgroundColor, QPen pen)
{
    this->backgroundColor = backgroundColor;
    this->backgroundPen = pen;
}

void PopUp::setPopupText(const QString &text, QString stylesheet)
{
...
void PopUp::show(int timeout)
{
    ...
    timer->start(timeout);             // А также стартуем таймер, который запустит скрытие уведомления через 3 секунды
}
